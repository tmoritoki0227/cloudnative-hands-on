...
- キーペアを作ってない人は先に作成してください。左メニューの`ネットワーク & セキュリティ`→`キーペア`をクリック。
  ...
  - 保存場所: 任意。一般的にはホームディレクトリの下の.sshディレクトリに配置します。
- インスタンスの作成（起動）
...
- インスタンスへのsshログイン
インスタンスへのログイン方法はいろいろありますが、ここではsshログインを紹介します。
  - インスタンスを選択し、画面上部の`接続`ボタン→ `SSHクライアント`でsshコマンドが表示されます。-iオプションで保存した秘密鍵を指定してください。
  - sshクライアントを用意してください。[powershell](https://mimimopu.com/powershell-ssh-client/)等
    ```
    $ cd 秘密鍵を保存したディレクトリ
    $ ssh -i "id_rsa.pem" ec2-user@ec2-3-113-4-139.ap-northeast-1.compute.amazonaws.com
    ```
  - インスタンスにsshログインしてください
- インスタンス削除（終了）
- 費用の確認方法
